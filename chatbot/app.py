...
def chat_api():
    ...
    try:
        ...
        # Adicionar mensagem do usuário ao histórico (por usuário)
        user_id = session.get('user_id')
        user_nome = session.get('user_nome') or session.get('username')
        session_manager.add_message(chat_id, user_message, "user", user_id=user_id, nome_usuario=user_nome)
        
        # Processar a mensagem e gerar resposta
        chat_history = session_manager.get_chat_history(chat_id, user_id=user_id)
        ai_response = process_message(user_message, chat_history)
        
        # Adicionar resposta do AI ao histórico
        session_manager.add_message(chat_id, ai_response, "ai", user_id=user_id)
...
def save_chat():
    ...
    try:
        ...
        if normalized_messages:
            user_id = session.get('user_id')
            session_manager.save_chat(chat_id, title, normalized_messages, user_id=user_id)
        else:
            if title:
                user_id = session.get('user_id')
...
def list_chats():
    ...
    try:
        user_id = session.get('user_id')
        chats = session_manager.list_chats(user_id=user_id)
...
def delete_chat():
    ...
    try:
        ...
        user_id = session.get('user_id')
        if session_manager.delete_chat(chat_id, user_id=user_id):
