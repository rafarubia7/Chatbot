...
from .horarios import formatar_horarios_para_prompt
...
"""
    ...
    def get_complete_info(self) -> str:
        ...
        # PRIORIDADE 2: Cursos e infraestrutura
        info += self.get_courses_info()
        info += self.get_infrastructure_info()
        
        # PRIORIDADE 3: Horários (importante para perguntas sobre salas/professores/turmas)
        info += formatar_horarios_para_prompt()
        
        # PRIORIDADE 4: Contatos e FAQ
        info += self.get_contacts_info()
        info += self.get_faq_info()
        
        # PRIORIDADE 5: Outras informações (se houver espaço)
    ...
    def get_contextual_info(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ['curso', 'cursos', 'técnico', 'superior', 'aprendizagem', 'qualificação']):
            ...
            # Incluir informações de horários quando perguntar sobre horários
            return formatar_horarios_para_prompt() + self.get_staff_info()
