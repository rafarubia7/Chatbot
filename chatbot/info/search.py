...
def obter_informacao_especifica(consulta: str) -> Optional[str]:
    ...
    # PRIORIDADE MÁXIMA: Tratamento de consultas com apenas números (busca direta de sala/banheiro)
    consulta_limpa = consulta.strip()
    if consulta_limpa.isdigit():
        # Buscar diretamente a sala/banheiro pelo número
...
"""
    ...
    # Resposta específica: OPP (Orientador de Prática Profissional)
    ...
    tem_opp_geral = any(trigger in consulta_normalizada for trigger in ['opp', 'orientador de pratica profissional', 'orientador de prática profissional'])
    tem_localizacao = any(palavra in consulta_normalizada for palavra in ['onde', 'fica', 'localização', 'localizacao', 'como chegar'])
    
    if tem_opp_geral and tem_localizacao:
        # Se menciona especificamente "sala opp", retornar apenas a sala do orientador
        if tem_sala_opp:
            # Retornar apenas a localização da sala do orientador (será tratada pela busca normal abaixo)
            pass  # Deixa passar para a busca normal que encontrará sala_orientador_rainer
        else:
            # Se perguntar apenas "opp" ou "orientador" sem "sala", mostrar as duas salas
            return (
                "O Orientador de Prática Profissional (OPP) tem acesso a duas salas:\n\n"
                "1. **Sala do Orientador (Rainer)**:\n"
                "   - Ao passar pela catraca da entrada, vire à esquerda e você verá uma rampa. Desça a rampa. Vire no primeiro corredor à sua direita. A sala do Orientador é a segunda porta à sua direita.\n\n"
                "2. **Sala de Preparação (215)**:\n"
                "   - Entre no refeitório e vire à esquerda. Siga em direção aos puffs. Entre no corredor à esquerda. A terceira porta à esquerda é a Sala de Preparação (215).\n\n"
                "O Orientador de Prática Profissional é o Rainer Messias Bruno (rainer.bruno@sp.senai.br)."
            )
    ...
    # PRIORIDADE: Perguntas sobre o endereço geral do SENAI São Carlos
    if any(palavra in consulta_lower for palavra in ['onde fica o senai', 'onde fica senai', 'endereço do senai', 'endereco do senai', 
                                                      'endereço senai', 'endereco senai', 'localização do senai', 'localizacao do senai',
                                                      'localização senai', 'localizacao senai', 'como chegar ao senai', 'como chegar senai',
                                                      'onde fica a escola', 'endereço da escola', 'endereco da escola']):
        ...
        return (
            f"📍 **Endereço do SENAI São Carlos:**\n\n"
            f"{endereco}\n\n"
            f"**Contatos:**\n"
            f"📞 Telefone/WhatsApp: {telefone}\n"
            f"📧 Email: {email}\n"
            f"🌐 Site: {site}\n\n"
            f"Posso te ajudar a encontrar algum local específico dentro da escola? Por exemplo, posso te indicar como chegar ao refeitório, biblioteca, laboratórios, etc. É só perguntar! 😊"
        )
    
    # PRIORIDADE: Verificação específica para Setor de Apoio, Julio, Carla e Fernanda (antes de outras verificações)
    ...
    if tem_palavra_localizacao:
        # Verificação específica para Setor de Apoio (prioridade máxima - antes de Carla/Fernanda)
        if any(palavra in consulta_normalizada for palavra in ['setor de apoio', 'setor apoio', 'sala de apoio', 'sala apoio', 'apoio']):
...
"""
                ...
                if sala.localizacao.sala:
                    resposta += f", Sala {sala.localizacao.sala}"
                # Adicionar informações sobre funcionários do setor
                from info.funcionarios import FUNCIONARIOS_SENAI_SAO_CARLOS
                setor_info = FUNCIONARIOS_SENAI_SAO_CARLOS.get('setor_apoio', {})
                if setor_info and setor_info.get('funcionarios'):
                    resposta += f"\n\nFuncionários responsáveis:"
                    for func in setor_info['funcionarios']:
                        resposta += f"\n- {func['nome']} ({func['cargo']}) - Horário: {func['horario']}"
                if sala.navegacao and sala.navegacao.dicas_adicionais:
                    resposta += f"\n\n{sala.navegacao.dicas_adicionais}"
                return resposta
        
        # Verificação específica para Julio/Coordenação
        if any(palavra in consulta_normalizada for palavra in ['julio', 'júlio', 'julio cesar', 'julio césar', 'julio cesar melli', 'julio césar melli', 'melli']):
            sala = SALAS.get('coordenacao_326')
            if sala:
                resposta = f"""Para chegar à Sala de Coordenação (326):

"""
                for instrucao in sala.navegacao.instrucoes:
                    resposta += f"- {instrucao}\n"
                resposta += (
                    f"\nLocalização: {sala.localizacao.predio}, {sala.localizacao.andar}, Sala {sala.localizacao.sala}\n"
                    f"Funcionário responsável: Julio Cesar Melli (Coordenador de Atividades Pedagógicas)"
                )
                return resposta
        
        # Verificação específica para Carla e Fernanda/Setor de Apoio
...
"""
    ...
    if tem_palavra_localizacao:
        ...
        palavras_chave_locais = {
            ...
            'setor_apoio': ['setor de apoio', 'apoio', 'qualidade de vida', 'analise de qualidade de vida', 'análise de qualidade de vida', 'sala 204', '204', 'carla', 'carla ballestero', 'fernanda', 'fernanda moreira'],
            ...
            'escada': ['escada'],
            'elevador': ['elevador'],
            'sala_preparacao_215': ['sala de preparação', 'sala de preparacao', '215', 'sala preparação', 'sala preparacao', 'preparação 215', 'preparacao 215'],
            ...
            'sala_orientador_rainer': ['sala do orientador', 'orientador', 'rainer', 'opp', 'orientador de prática profissional', 'orientador de pratica profissional', 'sala do opp', 'sala opp', 'opp rainer'],
            ...
            'lab_metrol_dimensional_131': ['lab metrologia dimensional', 'metrologia dimensional', 'metrologia', 'sala de metrologia', 'laboratório de metrologia', 'lab de metrologia', 'laboratorio de metrologia', 'sala metrologia', '131'],
            ...
            'oficina': ['oficina'],
            'uplab': ['uplab'],
            'coordenacao_326': ['coordenação', 'coordenacao', 'coordenação 326', 'coordenacao 326', 'sala 326', '326', 'coordenação pedagógica', 'coordenacao pedagogica', 'julio', 'júlio', 'julio cesar', 'julio césar', 'julio cesar melli', 'julio césar melli', 'melli']
        }
        
        # Verificação específica para OPP/Orientador de Prática Profissional (prioridade alta)
        if any(palavra in consulta_normalizada for palavra in ['opp', 'orientador de pratica profissional', 'orientador de prática profissional', 'sala do opp', 'sala opp']) and any(palavra in consulta_normalizada for palavra in ['onde encontro', 'onde fica', 'como chegar', 'sala']):
            melhor_id = 'sala_orientador_rainer'
        # Verificação específica para Setor de Apoio (prioridade máxima - antes de Carla/Fernanda)
        elif any(palavra in consulta_normalizada for palavra in ['setor de apoio', 'setor apoio', 'sala de apoio', 'sala apoio']) and any(palavra in consulta_normalizada for palavra in ['onde encontro', 'onde fica', 'como chegar', 'sala']):
            melhor_id = 'setor_apoio'
        # Verificação específica para Julio/Coordenação (prioridade alta)
        elif any(palavra in consulta_normalizada for palavra in ['julio', 'júlio', 'julio cesar', 'julio césar', 'julio cesar melli', 'julio césar melli', 'melli']) and any(palavra in consulta_normalizada for palavra in ['onde encontro', 'onde fica', 'como chegar', 'sala']):
            melhor_id = 'coordenacao_326'
        # Verificação específica para Carla e Fernanda/Setor de Apoio (prioridade alta)
            ...
            melhor_id = 'setor_apoio'
        # Verificação específica para metrologia (prioridade alta)
        elif any(palavra in consulta_normalizada for palavra in ['metrologia', 'sala de metrologia', 'laboratorio de metrologia', 'laboratório de metrologia', 'lab de metrologia']) and any(palavra in consulta_normalizada for palavra in ['onde encontro', 'onde fica', 'como chegar', 'sala']):
            melhor_id = 'lab_metrol_dimensional_131'
        # Verificação específica para coordenação (prioridade alta)
        elif any(palavra in consulta_normalizada for palavra in ['coordenacao', 'coordenação']) and any(palavra in consulta_normalizada for palavra in ['onde encontro', 'onde fica', 'como chegar']):
            melhor_id = 'coordenacao_326'
...
"""
    ...
    if any(palavra in consulta_normalizada for palavra in ['contato', 'telefone', 'email', 'e-mail', 'falar', 'comunicar']):
        # Verificação específica para email
        if any(palavra in consulta_lower for palavra in ['email', 'e-mail', 'correio eletronico', 'correio eletrônico']):
            email = INFO_SENAI_SAO_CARLOS.get('email', '')
            if email:
                return (
                    f"O email do SENAI São Carlos é: {email}\n\n"
                    f"Você pode entrar em contato pelo email para:\n"
                    f"- Informações sobre cursos\n"
                    f"- Processos de inscrição e matrícula\n"
                    f"- Dúvidas gerais sobre a unidade\n\n"
                    f"📞 Telefone/WhatsApp: {INFO_SENAI_SAO_CARLOS.get('telefone', '')}\n"
                    f"🌐 Site: {INFO_SENAI_SAO_CARLOS.get('site', '')}"
                )
    ...
    # Horário da biblioteca (PRIORIDADE ALTA)
    if any(p in consulta_lower for p in ['horário', 'horario', 'horários', 'horarios']) and any(p in consulta_lower for p in ['biblioteca', 'bibliote']):
        horario_biblioteca = INFO_SENAI_SAO_CARLOS.get('horario_biblioteca', '')
        if horario_biblioteca:
            return (
                f"📚 **Horários da Biblioteca do SENAI São Carlos:**\n\n"
                f"{horario_biblioteca}\n\n"
                f"Para mais informações sobre a biblioteca ou consultar o acervo, entre em contato:\n"
                f"📞 Telefone: {INFO_SENAI_SAO_CARLOS.get('telefone', '')}\n"
                f"📧 Email: {INFO_SENAI_SAO_CARLOS.get('email', '')}"
            )
        else:
            return (
                "📚 **Horários da Biblioteca do SENAI São Carlos:**\n\n"
                "Segunda a quinta-feira: das 8h30 às 13h30 e das 15h às 22h\n"
                "Sextas-feiras: das 8h30 às 13h30 e das 15h às 21h\n"
                "Sábados: das 8h às 12h15 e das 12h30 às 14h15\n\n"
                f"Para mais informações, entre em contato:\n"
                f"📞 Telefone: {INFO_SENAI_SAO_CARLOS.get('telefone', '')}\n"
                f"📧 Email: {INFO_SENAI_SAO_CARLOS.get('email', '')}"
            )
    ...
    # Verificação específica para Sala de Preparação (215) - prioridade sobre CST
    if ('sala de preparação' in consulta or 'sala de preparacao' in consulta or 'preparação 215' in consulta or 'preparacao 215' in consulta) and 'cst' not in consulta_normalizada:
        return (
            "Para chegar à Sala de Preparação (215):\n\n"
            ...
            "Localização: Principal, Andar Inferior, Sala 215\n\n"
            "Esta é uma das salas às quais o Orientador de Prática Profissional (OPP) tem acesso."
        )
    ...
    # MAS: não interceptar se for pergunta sobre "quem trabalha" (deve ir para LM Studio)
    ...
    for id_sala, palavras in palavras_chave_locais_direto.items():
        # NÃO retornar localização se for pergunta sobre "quem trabalha" sem palavra de localização
        if any(palavra in consulta_lower for palavra in ['quem trabalha', 'quem é', 'quem está', 'quem esta']):
            if not any(palavra in consulta_lower for palavra in ['onde fica', 'onde está', 'onde esta', 'localização', 'localizacao', 'como chegar']):
                continue  # Pular esta verificação, deixar para LM Studio
...
"""
    ...
    # Perguntas sobre microondas e geladeira (PRIORIDADE ALTA - antes da descrição do refeitório)
    if any(palavra in consulta_lower for palavra in ['microondas', 'micro-ondas', 'micro ondas', 'geladeira', 'geladeiras']):
        # Se for pergunta de localização (onde fica, onde está, etc.) ou apenas mencionar o equipamento
        tem_palavra_localizacao = any(palavra in consulta_lower for palavra in ['onde fica', 'onde está', 'onde esta', 'localização', 'localizacao', 'como chegar', 'onde encontro', 'onde acho'])
        # Se mencionar o equipamento e for pergunta de localização OU se a pergunta é curta (provavelmente sobre localização)
        if tem_palavra_localizacao or len(consulta_lower.split()) <= 5:
            return (
                "Os micro-ondas e geladeiras ficam no refeitório.\n\n"
                ...
                "Localização: Prédio Principal, Térreo\n\n"
                "Ao entrar no refeitório, você encontrará os micro-ondas e geladeiras disponíveis para uso."
            )
    ...
    # PRIORIDADE: Perguntas sobre "quem trabalha", "quem é", "quem está" devem ir para LM Studio
    # ANTES de qualquer busca de localização
    perguntas_quem = ['quem trabalha', 'quem é', 'quem está', 'quem esta', 'quem são', 'quem sao']
    # Verificar se é pergunta sobre pessoas (não localização)
    tem_palavra_localizacao_quem = any(palavra in consulta_lower for palavra in ['onde fica', 'onde está', 'onde esta', 'localização', 'localizacao', 'como chegar'])
    if any(pergunta in consulta_lower for pergunta in perguntas_quem) and not tem_palavra_localizacao_quem:
        # Deixar para o LM Studio responder (perguntas sobre pessoas/funcionários)
        return None
